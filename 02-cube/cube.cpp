...
class ExampleCube : public entry::AppI {
public:
	ExampleCube(const char* _name, const char* _description, const char* _url)
    ...
    void init(int32_t _argc, const char* const* _argv, uint32_t _width, uint32_t _height) override {
        ...
        // load shader
        program_ = loadProgram("vs_cube", "fs_cube");
    ...
    bool update() override {
        if (!entry::processEvents(width_, height_, debug_, reset_, &mouse_state_) ) {
            ...
            // transform matrix
            float rotate_mtx[16];
            bx::mtxRotateXYZ(rotate_mtx, 3.14 / 8 + time, 0, 3.14 / 4);
            // bx::mtxRotateXYZ(rotate_mtx, 3.14 / 8, 3.14 / 8, 0);

            float scale_mtx[16];
            bx::mtxScale(scale_mtx, 5);

            float transform_mtx[16];
            bx::mtxMul(transform_mtx, scale_mtx, rotate_mtx);

            bgfx::setTransform(transform_mtx);
